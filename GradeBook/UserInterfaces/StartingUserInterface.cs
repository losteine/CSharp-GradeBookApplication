...
namespace GradeBook.UserInterfaces
{
    public static class StartingUserInterface
    {
        ...
        public static void CreateCommand(string command)
        {
            ...
            var name = parts[1];
            var type = parts[2];
            bool isValidBool = bool.TryParse(parts[3], out isValidBool);
            bool weighted;
            if(isValidBool == true)
            {
            ...
            {
                Console.WriteLine($"{parts[3]} is not a valid command. Please specify whether the gradebook is weighted (true) or not (false)");
                return;
            }
